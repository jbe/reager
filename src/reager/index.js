...
export function getUserPhotoOrGravatar(
  firebaseUser,
  size = 100,
  generator = "monsterid"
) {
  const { photoURL, email, emailVerified } = firebaseUser;
  if (photoURL) return photoURL;
  return (
    getGravatarImage(emailVerified ? email : "") +
    "?d=" +
    generator +
    "&s=" +
    size
...
export function useReplacePath() {
  ...
  const modified = useCallback(
    paramsOverride => toPath({ ...params, ...paramsOverride }),
    [toPath, params]
  );
  return modified;
}
...
export function explainFirebaseError(error) {
  var message = {
    ...
    "auth/too-many-requests":
      "Too many failed attempts. Please wait a short while and try again."
  }[error.code];
